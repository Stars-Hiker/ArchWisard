...
# IMPORTANT: _tty() returning "1" and then using > "$(_tty)" creates a FILE
# named "1", NOT a redirect to fd 1.  When gum's stdout becomes a regular file
# it internally falls back to /dev/tty for rendering but can no longer cleanly
# read back OSC query responses — they leak into the tty input buffer and the
# shell echoes them as garbage (^[]11;rgb:…  ^[[50;1R).
#
# Fix: _gum_show() avoids any redirect when stdout is already a tty.
# Use it for every display (non-interactive) gum call.
_gum_show() {
    # inputs: gum command + args | side-effects: renders to terminal
    if [[ -t 1 ]]; then
        "$@" 2>/dev/null || true          # stdout is a real tty — write normally
    elif [[ -w /dev/tty ]]; then
        "$@" > /dev/tty 2>/dev/null || true  # stdout is piped (tee) — route to tty
    else
        "$@" 2>/dev/null || true
    fi
}
...
# ── OSC-safe display helpers ──────────────────────────────────────────────────
_ok()   { _gum_show gum style --foreground $C_OK   " ✔  $*" || printf ' ✔  %s\n' "$*"; }
_info() { _gum_show gum style --foreground $C_INFO " ℹ  $*" || printf ' ℹ  %s\n' "$*"; }
_warn() { _gum_show gum style --foreground $C_WARN " ⚠  $*" || printf ' ⚠  %s\n' "$*"; }
