...
## Project Description
ArchWizard is a modular Arch Linux installer in Bash + gum.
Goal: the most reliable, versatile, user-friendly, and beautiful Arch installer possible.
...
## File Map
```
archwizard.sh                      — entry point, globals, source order
lib/
  ui.sh                            — all gum wrappers + NO_GUM fallbacks
  state.sh                         — step machine, menu, _exec_install
  detect.sh                        — env detection, keyboard, shared helpers
  disk.sh                          — disk survey, OS probe, space planning
  storage.sh                       — storage stack wizard (new step 3)
  layout.sh                        — partition sizing, swap, layout preview
  identity.sh                      — system ID, users, kernel/boot, DE, summary
  partition.sh                     — replace/resize/create partitions, LUKS
  format.sh                        — mkfs, btrfs subvols, mount tree
  base.sh                          — pacstrap, fstab, mirrorlist
  chroot_gen.sh                    — serialize state, deploy template, run chroot
chroot/
  bootloader.sh                    — EFI guard, fallback copy, NVRAM check
  desktop.sh                       — dotfiles deploy (git/chezmoi/local)
  postinstall.sh                   — verification, cleanup, reboot
templates/
  chroot_base.sh                   — real bash chroot script (not heredoc)
