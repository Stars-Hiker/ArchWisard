...
#  Version : 5.5.0-gum-1.3.1
#  License : MIT
#  Depends : gum (https://github.com/charmbracelet/gum)
#  Usage   : bash archwizard_gum.sh [--dry-run] [--verbose] [--load-config FILE]
#
#  Changes in 1.3.1  (bug-fix release)
#    [fix] ROOT CAUSE — bare [[ ]] && / [[ ]] || patterns return exit 1 when the
#          condition is false; set -e then kills the script silently before any
#          gum call is even reached.  All such patterns converted to if/then/fi.
#          Affected sites: parse_args, main, _check_and_plan_space, select_disks,
#          and several loop bodies (candidate builders, other_disks).
...
# ── _get_gb_gum ───────────────────────────────────────────────────────────────
# Validated numeric GB input. Sets global GB_RESULT. Loops until valid.
_get_gb_gum() {
...
_layout_preview() {
    ...
    if [[ "$ROOT_SIZE" == "rest" ]]; then root_disp="all remaining space"; fi
    local luks_tag=""
    ...
    if [[ "$SEP_HOME" == true && "$DISK_HOME" == "$DISK_ROOT" ]]; then
        ...
        if [[ "$HOME_SIZE" == "rest" ]]; then home_disp="all remaining space"; fi
...
partition_wizard() {
    ...
    if [[ "$FIRMWARE_MODE" == "bios" ]]; then
        ...
        if [[ "$REUSE_EFI" == false || -z "$EFI_PART" ]]; then
            ...
            if [[ -n "$_esp_found" ]]; then
                ...
                warn "No EFI System Partition found — a new 512 MB one will be created."
        ...
        local efi_input
        efi_input=$(input_gum "EFI partition size in MB  (recommended: 512)" "512")
    ...
    case "$layout_choice" in
        root_only)
            if confirm_gum "Use all ${avail_gb} GB for / ? (recommended)"; then
                ROOT_SIZE="rest"
            else
        ...
        root_home|root_home_swap)
            ...
            if (( avail_gb < 60  )); then suggested_root=25; fi
            if (( avail_gb < 30  )); then suggested_root=15; fi
            if (( avail_gb < 15  )); then suggested_root=$(( avail_gb * 6 / 10 )); fi
            if (( avail_gb > 100 )); then suggested_root=60; fi
            gum style \
            ...
            blank
            _get_gb_gum "Root (/) size in GB" "$suggested_root" "$(( avail_gb - 4 ))"
            ROOT_SIZE="$GB_RESULT"
        ...
        split_disk)
            ...
            _get_gb_gum "Root (/) size in GB  [on ${DISK_ROOT}]" "$root_default" "$root_max"
            ROOT_SIZE="$GB_RESULT"
            ...
            if confirm_gum "Give all ${home_gb} GB to /home?"; then
                ...
                _get_gb_gum "Home (/home) size in GB  [on ${DISK_HOME}]" "$home_gb" "$home_gb"
                HOME_SIZE="$GB_RESULT"
            fi
            ;;
    esac
    ...
    case "${swap_sel%% *}" in
        Swap\ file*|file*)
            ...
            if [[ $sf_max -lt 1 ]]; then sf_max=1; fi
            _get_gb_gum "Swap file size in GB" "${recommended_swap:-8}" "$sf_max" 1
            SWAP_SIZE="$GB_RESULT"
            ;;
        Swap\ partition*|partition*)
            SWAP_TYPE="partition"
            local sp_max=$(( disk_gb / 4 ))
            if [[ $sp_max -lt 1 ]]; then sp_max=1; fi
            _get_gb_gum "Swap partition size in GB" "${recommended_swap:-8}" "$sp_max" 1
            SWAP_SIZE="$GB_RESULT"
            ;;
        None*)
        ...
        *)
            SWAP_TYPE="zram"
            local zram_pct
            zram_pct=$(input_gum "zram size as % of RAM  (e.g. 100)" "100")
            if [[ "$zram_pct" =~ ^[0-9]+$ ]] && (( zram_pct > 0 && zram_pct <= 200 )); then
                SWAP_SIZE="${zram_pct}%"
            else
                SWAP_SIZE="100%"
            fi
...
configure_system() {
    ...
    blank
    gum style \
        --border rounded --border-foreground "$GUM_C_OK" \
        --padding "0 2" --width "$GUM_WIDTH" \
...
configure_users() {
    ...
    info "Username: lowercase letters, digits, underscores, hyphens. Start with a letter."
    blank
    while true; do
    ...
    ok "Username: ${USERNAME}  (wheel/sudo group)"
    blank
    ...
    info "Root account password — emergency console access."
    blank
