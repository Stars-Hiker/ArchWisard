...
# ── EXIT trap — write final summary to log ────────────────────────────────────
trap 'echo "[$(date +%H:%M:%S)] EXIT code=$?" >> "$LOG_FILE"' EXIT
...
# =============================================================================
#  LOGGING
#  We do NOT redirect global stdout to tee — that would make gum's isatty()
#  return false, disabling all ANSI rendering (borders, colours, spinners).
#  Instead, log() dual-writes, and run() pipes command output through tee.
# =============================================================================
LOG_FILE="/tmp/archwizard.log"
# Initialise / truncate the log file so it exists from the start
: > "$LOG_FILE"
